...
class NetEase:
    ...
    def songs_detail(self, ids, offset=0):
        # NOTE 163 api won't return songs list in accurate sequence as the ids we passed
        tmpids = ids[offset:]
        tmpids = tmpids[0:100]
    ...
    def song_lyric(self, music_id):
        action = 'http://music.163.com/api/song/lyric?os=osx&id=%s&lv=-1&kv=-1&tv=-1' % str(music_id)
        try:
            data = self.httpRequest('GET', action)
            return data['songs']
        except:
